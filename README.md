...
| [memory-backup](plugins/memory-backup) | `/backup` (+ `setup`, `status`, `schedule`)<br>`/backup restore`<br>`/backup merge`<br>`/backup zip <path>`<br>`/backup share <thread>` | Mirrors every memory store, handoff note, plan, and hand-written global config to a private GitHub repo, one squash-merged PR per run; visibility verified and every file secret-scanned before each push, so credentials never leave the machine. Restore fills what's missing and asks on conflicts; merge converges any number of machines; zip exports a portable dated archive, no GitHub needed; share hands one thread of work (its handoff note, the memory it touches, the plans you pick, a generated onboarding page) to a colleague without repo access, as a secret gist or a zip. Can also target an S3-compatible bucket (AWS S3, GCS, Alibaba OSS, MinIO) alongside or instead of GitHub, with the same secret scan and a refuse-if-public gate. Scheduling optional (cron or launchd), weekly default. |
